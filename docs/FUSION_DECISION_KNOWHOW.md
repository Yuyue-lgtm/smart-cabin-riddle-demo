...
V1.2 部署时，上述职责不等于必须拆成多个串行大模型节点。安全判断、精确猜中、乘客候选过滤和 UI 组装应使用代码节点；普通问题的判断、主持话术和可选乘客动作合并为一次模型调用，具体见 [COZE_FAST_PATH_OPTIMIZATION.md](./COZE_FAST_PATH_OPTIMIZATION.md)。
