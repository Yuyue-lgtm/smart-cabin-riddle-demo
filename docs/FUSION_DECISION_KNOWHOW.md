...
本文中的策略和话术首先是“能力愿景与典型案例集”，不是要求 Workflow 把每一条案例写成独立分支，也不是每次请求都把全文发送给大模型。

Know-how 在运行时分成三层：

1. **常驻主持原则**：安全优先、一次只处理一个主要目标、不要机械播报感知信息、不要打扰睡着乘客等。内容短小，常驻模型系统提示词。
2. **策略索引**：安全接管、游戏推进、邀请沉默玩家、环境融合、胜利造神等通用主持手段。由代码节点根据当前局势快速筛选。
3. **完整经验库**：本文中的典型场景、表达技巧和正反例。只按当前策略召回最相关的 2-3 条；第一阶段可由代码映射，规模扩大后可迁移到知识库。

因此，Know-how 可以持续扩展，而不会线性增加每次请求的上下文和延迟。

## V1.2 最小感知范围

第一版只接入观众能够快速看懂、且能明显体现主持人眼力见的感知数据：

| 感知 | 标准状态 | 主持人可感知行为 |
| --- | --- | --- |
| 车辆风险 | `normal` / `high_speed` / `hard_brake` | 急刹立即暂停并安抚；高速降低主驾互动 |
| 主驾疲劳 | `normal` / `fatigued` | 不要求主驾答题，转交副驾和后排 |
| 乘客睡眠 | `normal` / `sleeping` | 不再 cue，也不生成该座位的模拟发言 |
| 乘客参与度 | `normal` / `inactive` | 在安全且游戏节奏合适时自然邀请参与 |
| 游戏进展 | `normal` / `stuck` / `near_answer` / `correct` | 分别正常主持、给提示、造势、庆祝 |
| 车外环境 | 沿用黄金体验线环境枚举 | 将雨天、雪景、高速等自然融入提示或话术 |
| 舱内气氛（可选） | `normal` / `laughing` | 对有创意的答案顺势接梗 |

乘客年龄、关系和目的地由黄金体验线预设，不作为实时手动感知项。接电话、冲突、长期记忆、真实声纹和精细情绪识别暂不进入 V1.2。
...
本文的策略表同时承担策略索引和案例来源。模型运行时只接收当前局势相关的少量策略与案例，不全量读取本文。
...
1. `S01` 急刹打断与自动恢复
2. `S02` / `S03` 高速和主驾疲劳时降低主驾互动
3. `S04` / `S25` 睡着乘客不被 cue、也不再模拟发言
4. `S21` 长期未参与乘客的自然邀请
5. `S17` / `S18` 游戏接近答案时造势、卡住时给提示
6. `S07` / `S08` 雨天、雪景等环境自然融入主持
7. `S14` / `S15` / `S16` 猜对后的差异化高情绪价值庆祝

`S20` 大笑或有创意答案的顺势接梗作为低成本可选增强。后续策略继续保留在案例库中，但不要求第一版全部接入。
...
根据局势选择一个主要策略，最多附加两个表达修饰因素。安全策略由规则确定，普通体验策略可用评分选择；不要为了显示融合而同时执行多个互相竞争的动作。
...
根据 `primary_strategy` 和 `modifiers` 取得少量相关经验：

- 第一阶段：代码节点从本地策略映射中选择 2-3 条原则或案例，延迟最低。
- 扩展阶段：案例数量明显增加后，使用知识库按标签或语义检索 Top 2-3；安全规则和常驻原则不依赖知识库。
- 未命中案例时：只使用常驻主持原则，不因召回失败阻塞请求。

#### Fast Director

一次完成游戏理解、主持话术和可选的一个模拟乘客动作，不再串联多个大模型节点。
